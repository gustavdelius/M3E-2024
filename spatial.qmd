...
We now come to a very intriguing topic: how the complicated spatial patterns one can observe in nature can arise. Whereas one might have thought that to explain complicated patterns one would need complicated models that encode all that complexity, it turns out that patterns can arise from very simple, translation-invariant models. The key idea is that the spatially uniform steady state in a translation-invariant model can be unstable to small perturbations, which then grow and form spatial patterns. This is called a Turing instability.

We consider a system of two species, described by densities $u(x,t)$ and $v(x,t)$, that interact with each other locally but also move around randomly. The system is described by the equations
$$\begin{split}
...
$u(x,t)=u^*$, $v(x,t)=v^*$
is a solution of the system. Assume that in the absence of diffusion, this steady state is stable. Thus without diffusion, any perturbations to the spatially homogeneous steady state will decay. One would then expect that the spatially homogeneous steady state is stable also in the presence of diffusion, given that diffusion has the tendency to spread individuals out away from regions of higher concentration. However, we will now see that this is not always the case. Rather unintuitively, the random motion of the individuals can destabilise the spatially homogeneous steady state and give rise to spatial patterns.

### Deriving conditions for Turing instabilities

Mathematics is a great tool for dealing with non-intuitive phenomena. We just have to do the maths and see what it tells us. So we now derive the equations that describe the time-evolution of small perturbations and solve them and see whether the perturbations grow or not. So we set
$$
...
We now use Taylor expansions to linearise the equations. We expand $f$ and $g$ around $(u^*,v^*)$ to first order and get
$$\begin{split}
...
&=a_{21}\xi+a_{22}\eta+D_1\frac{\partial^2\xi}{\partial x^2}.
...
$$
\xi(x,t)=B_1e^{\sigma_k t}\sin(kx+\alpha),~~~\eta(x,t)=B_2e^{\sigma_k t}\sin(kx+\alpha).
$$ {#eq-turingpert4}
Plugging this into @eq-turingpert3 and dividing by $e^{\sigma_k t}\sin(kx+\alpha)$ we get
$$\begin{split}
...
where
$$
A(k)=\begin{pmatrix}a_{11}-k^2D_1&a_{12}\\a_{21}&a_{22}-k^2D_2\end{pmatrix}.
$$ {#eq-turingpert7}
...
$$
We are interested in the case where the steady state is stable in the absence of diffusion and is destabilised by the diffusion. Stability in the absence of diffusion requires $\text{tr}(A(0))<0$. We see that in that case also $\text{tr}(A(k))<0$ for all $k$ because the $k$-dependent term $-k^2(D_1+D_2)$ is negative.
...
Stability in the absence of diffusion tells us that $\det(A(0))>0$. We see that $\det(A(k))$ is a quadratic polynomial in $k^2$ with positive leading coefficient. Its graph is a parabola. This means that the determinant is positive for small $k$ and for large $k$ but there may be an intermediate region where the parabola dips below the axis and the determinant is negative in that region. That region is bounded by values $k_\pm$ for which the determinant is zero. Using the quadratic formula we find that
$$
...
We have now derived the conditions for Turing instabilities in terms of the entries of the Jacobian matrix of the local dynamics of the two species and the diffusion coefficients:
$$\begin{split}
...
$$ {#eq-turingjac}
If the signs in the Jacobian are different then you can immediately rule out a Turing instability.

Next we look at what our conditions a) to d) tell us about the magnitude of the diffusion coefficients. From condition a) we know that $a_{11}<-a_{22}$ and hence $-a_{11}/a_{22}<1$. From condition c) we see that $D_1/D_2<-a_{11}/a_{22}$. Putting these together gives $D_1/D_2<1$ and hence $D_1<D_2$: The inhibitor must diffuse faster than the activator. This is a necessary condition for a Turing instability. Exactly how much faster the inhibitor must diffuse than the activator depends on the details of the local interactions in particular examples.

### Example

The Schnakenberg model is a simple model that exhibits Turing instabilities. It is a model of two species $u$ and $v$ described by
$$\begin{split}
...
$$ {#eq-schnakjac}
We are interested in the coexistence steady state $(u^*,v^*)$ where both species coexist. By imposing that $f(u^*,v^*)=0$ gives us that $u^*v^*=1$ and then imposing that $g(u^*,v^*)=0$ gives us that $u^*=b$ and hence $v^*=1/b$. So the coexistence steady state is
$$
...
$$ {#eq-schnakjac2}
We see that the sign structure of this matrix is one of those that can give rise to a Turing instability. Species $u$ is the activator and species $v$ is the inhibitor.

Now let us check each of the conditions a) to d) from @eq-turingconditions. Condition a) becomes $1-b^2<0$ and hence $b^2>1$. Condition b) becomes $-b^2+2b^2>0$ which is automatically satisfied. Condition c) becomes $D_2-b^2D_1>0$ which implies $D_1b^2<D_2$ or that the ratio $d=D_2/D_1>b^2$. Condition d) becomes $(D_2-b^2D_1)^2>4D_1D_2b^2$. Multiplying out the square and bringing everything to one side gives
$b^4D_1^2-6D_1D_2+D_2^2>0.$. We can rewrite this in terms of the ratio $d=D_2/D_1$ as
$$
b^4-6db^2+d^2>0.
$$ {#eq-schnakcond}
The graph of the left-hand side is a parabola that opens upwards and has roots at
$$
d_\pm=3b^2\pm\sqrt{9b^4-b^4}=b^2(3\pm\sqrt{8}).
$$ {#eq-schnakd}
The condition in @eq-schnakcond tells us that $d$ is outside the interval $(d_-,d_+)$. However we already also derived that $d>b^2$ and $b^2>d_-$. So the final condition on the ratio of the diffusion rates is
$$
...
$$ {#eq-chemotaxis}
where $\chi<0$ is the chemotactic sensitivity. Let's compare this with the diffusive flux
$$
...
The equations for the slime mould aggregation are then
$$\begin{split}
\partial_t n&=-\chi\partial_x(n\partial_x a)+D_n\partial_x^2 n,\\
\partial_t a&=hn-q a+D_a\partial_x^2 a.
\end{split}$$ {#eq-slime}
The homogeneous steady state where $n(x,t)=n^*, a(x,t)=a^*$ has to satisfies $h n^*=q a^*$. We can now linearise the equations around the homogeneous steady state by setting $n(x,t)=n^*+\xi(x,t)$ and $a(x,t)=a^*+\eta(x,t)$ with $\xi$ and $\eta$ small. We get
$$\begin{split}
...
$$
\text{tr}=\text{tr}(A(k))=-(D_n+D_a)k^2-q.
$$ {#eq-slimetrace}
