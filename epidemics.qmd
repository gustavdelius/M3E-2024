...
We can represent the SIR model graphically by a directed graph with one node for each component and directed edges between the nodes to represent the flow of individuals between the compartments. This is illustrated in @fig-sir. We have labeled the edges by the per-capita rates.
...
Vaccination is a way to move individuals directly from the S component to the R component. This is illustrated in @fig-vaccination .
...
$$ {#eq-ImaxV}
where we have recognised the right-hand side as the expression for the peak of the epdidemic in the absence of vaccinations.
...
$$ {#eq-Istar}
This is the endemic steady state of the SIR model with loss of immunity. We see that the number of infecteds at the endemic steady state is reduced by the loss of immunity rate $\mu$.
